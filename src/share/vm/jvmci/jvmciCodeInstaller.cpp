...
int AOTOopRecorder::find_index(Metadata* h) {
  ...
  assert(index + 1 == newCount, "must be last");
...
void* CodeInstaller::record_metadata_reference(CodeSection* section, address dest, JVMCIObject constant, JVMCI_TRAPS) {
  ...
  if (jvmci_env()->isa_HotSpotResolvedObjectTypeImpl(obj)) {
    ...
    assert(!jvmci_env()->get_HotSpotMetaspaceConstantImpl_compressed(constant), err_msg("unexpected compressed klass pointer %s @ " INTPTR_FORMAT, klass->name()->as_C_string(), p2i(klass)));
    ...
    assert(!jvmci_env()->get_HotSpotMetaspaceConstantImpl_compressed(constant), err_msg("unexpected compressed method pointer %s @ " INTPTR_FORMAT, method->name()->as_C_string(), p2i(method)));
...
void CodeInstaller::record_object_value(ObjectValue* sv, JVMCIObject value, GrowableArray<ScopeValue*>* objects, JVMCI_TRAPS) {
  ...
  for (jint i = 0; i < JVMCIENV->get_length(values); i++) {
    ...
    if (isByteArray && cur_second != NULL && (type == T_DOUBLE || type == T_LONG)) {
      // we are trying to write a long in a byte Array. We will need to count the marked entries to restore the type of
