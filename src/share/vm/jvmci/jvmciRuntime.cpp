...
JRT_ENTRY_NO_ASYNC(static address, exception_handler_for_pc_helper(JavaThread* thread, oopDesc* ex, address pc, nmethod*& nm))
  ...
  if (TraceExceptions) {
    ttyLocker ttyl;
    ResourceMark rm;
    tty->print_cr("Exception <%s> (" INTPTR_FORMAT ") thrown in JVMCI compiled method <%s> at PC " INTPTR_FORMAT " for thread " INTPTR_FORMAT "",
...
void JVMCINMethodData::add_failed_speculation(nmethod* nm, jlong speculation) {
  ...
  guarantee(index >= 0 && index <= max_jint, err_msg("Encoded JVMCI speculation index is not a positive Java int: " INTPTR_FORMAT, index));
  int length = speculation & JVMCINMethodData::SPECULATION_LENGTH_MASK;
  if (index + length > (uint) nm->speculations_size()) {
    fatal(err_msg(INTPTR_FORMAT "[index: " JLONG_FORMAT ", length: %d] out of bounds wrt encoded speculations of length %u", speculation, index, length, nm->speculations_size()));
  }
...
static void _fatal() {
  ...
  fatal(err_msg("thread " INT64_FORMAT ": Fatal error in JVMCI shared library", current_thread_id));
}
